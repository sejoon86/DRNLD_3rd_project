...
BATCH_SIZE = 128      # minibatch size  (1024)
...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed):
        ...
        self.actor_local_0 = Actor(state_size, action_size, random_seed).to(device)
        self.actor_target_0 = Actor(state_size, action_size, random_seed).to(device)
    ...
    def act(self, state_0, state_1, add_noise=True):
        """Returns actions for given state as per current policy."""
        state_0 = torch.from_numpy(state_0).float().to(device)
        state_1 = torch.from_numpy(state_1).float().to(device)
        self.actor_local_0.eval()
        self.actor_local_1.eval()
        with torch.no_grad():
            action_0 = self.actor_local_0(state_0).cpu().data.numpy()
            action_1 = self.actor_local_1(state_1).cpu().data.numpy()
        self.actor_local_0.train()
        self.actor_local_1.train()
        if add_noise:
            action_0 += self.noise.sample()
            action_1 += self.noise.sample()
                  
        
        action_0 = np.expand_dims(action_0,axis=0)
    ...
    def learn(self, experiences, gamma, agent_num):
        ...
        # Compute actor loss
        if agent_num ==0:
            actions_pred = self.actor_local_0(states)
        ...
        if agent_num == 0:
            self.soft_update(self.actor_local_0, self.actor_target_0, TAU)                     
        else:
